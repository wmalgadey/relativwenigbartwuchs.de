...
categories:
  - Webseite
status: gefunden
source: https://blog.esciencecenter.nl/why-all-youll-ever-need-is-markdown-dc604f0ab309
tags:
  - Markdown
  - Pandoc
area:
...
---
[![](https://miro.medium.com/max/1200/0*4G2GgU10iJkPldd1)](https://miro.medium.com/max/1200/0*4G2GgU10iJkPldd1)
...
There are many ways to cite papers, the syntax for which is [documented in the Pandoc manual.](https://pandoc.org/MANUAL.html#citations) Basic citation looks like `[@Hidding2014]` , where `Hidding2014` is an entry in the bibliography. Several editors (VSCode and Emacs) even support autocompletion on the included bibliography.
...
Yes, this is [Haskell](https://haskell.org/) syntax. It just means what you think it means. Using native output in Pandoc will be very useful if you start developing your own filters. For now it just serves to illustrates how Pandoc works. If you’re not a hacker, you’ll never have to look at this again 🤓👍.
...
There is a big problem with the above example. The equation is not numbered! Pandoc filters let you change the intermediate representation. Let’s try the [`pandoc-eqnos`](https://github.com/tomduck/pandoc-eqnos) filter. This filter is written in Python (using `pypandoc`); any language that can read the intermediate JSON representation works.
...
```
The prove of inexistence of a solution forEquation @eq:fermat had eluded mathematicians for centuries.
```
...
This was just a teaser of what is possible with Pandoc. Did I mention creating slide shows with `reveal.js` ? Or how about doing some literate programming with `entangled` [(see my previous post on that)?](https://blog.esciencecenter.nl/entangled-1744448f4b9f) The documentation of Pandoc is excellent, so just go ahead and write all your content in Markdown!