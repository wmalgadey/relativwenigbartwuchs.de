---
source: https://github.com/talex5/cuekeeper
---
# CueKeeper

Copyright Thomas Leonard, 2020

CueKeeper is a web-based [GTD](https://en.wikipedia.org/wiki/Getting_Things_Done) system (a fancy TODO list) that runs entirely in your browser (the data is stored on your computer, in your browser).

![[CueKeeper-0.2.png]]

## Installation

Download the latest `cuekeeper-bin` zip from the [releases list](https://github.com/talex5/cuekeeper/releases), e.g.

[https://github.com/talex5/cuekeeper/releases/download/v0.4/cuekeeper-bin-0.4.zip](https://github.com/talex5/cuekeeper/releases/download/v0.4/cuekeeper-bin-0.4.zip)
...
[http://roscidus.com/blog/blog/2015/04/28/cuekeeper-gitting-things-done-in-the-browser/](http://roscidus.com/blog/blog/2015/04/28/cuekeeper-gitting-things-done-in-the-browser/)
...
You'll need the [opam](http://opam.ocaml.org/) package manager. It should be available through your distribution, but you can use a [generic opam binary](http://tools.ocaml.org/opam.xml) if it's missing or too old (I use opam 2.0.4). Ensure you're using OCaml 4.07.1 (check with `ocaml -version`). If not, switch to that version:
...
First, generate an access token (a _long_ random string that grants access to the server). The `pwgen` command is useful for this:
...
Finally, you should be prompted for your access key. Paste in the token you generated above (e.g. `dtXZ7fQfX52VsnJNk22J6uKy8JSn6klb` in the example above - _not_ the hash).

## Deploying as a Xen VM

In fact, the server is a [Mirage unikernel](http://openmirage.org/) and can also be compiled and booted as a Xen virtual machine:
...
[http://lists.xenproject.org/cgi-bin/mailman/listinfo/mirageos-devel](http://lists.xenproject.org/cgi-bin/mailman/listinfo/mirageos-devel)

Bugs can be reported on the mailing list or as GitHub issues:

[https://github.com/talex5/cuekeeper/issues](https://github.com/talex5/cuekeeper/issues)
...
This project includes Foundation ([http://foundation.zurb.com](http://foundation.zurb.com/)). These files are released under the MIT license.

This project includes the Pikaday date picker ([https://github.com/dbushell/Pikaday](https://github.com/dbushell/Pikaday)). These files are released under the BSD & MIT licenses.

This project includes FileSaver.js ([https://github.com/eligrey/FileSaver.js](https://github.com/eligrey/FileSaver.js)), which is released under a permissive license.
