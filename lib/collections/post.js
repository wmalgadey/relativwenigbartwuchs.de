...
export default (collection, { dev, now }) => {
  ...
  postFiles.forEach(filePath => {
    ...
    if (!dev && new Date(data['date created']) > now) return;

    const title = data.title || '';
    const fileSlug = slugify(path.basename(filePath, path.extname(filePath)), { decamelize: false });
    const url = `/${fileSlug}/`;
  ...
  log.info(`Created ${posts.length} posts`, 'collection');
