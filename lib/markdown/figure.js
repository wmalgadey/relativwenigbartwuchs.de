...
const log = new ConsoleLogger('figure');
...
export default md => {
  ...
  md.renderer.rules.image = function (tokens, idx, options, env, self) {
    ...
    const inputBaseDir = path.join(path.dirname(inputPath), inputStem);
    ...
    if (parsed.skip || imgSrc.startsWith('http')) {
      log.info(`Skipping image processing for ${imgSrc} (reason: ${parsed.skip ? 'skip flag' : 'external URL'})`);
    ...
    log.info(`Processing image ${imgSrc} with 11ty Image`);
    ...
    const src = path.join(inputBaseDir, imgSrc);
