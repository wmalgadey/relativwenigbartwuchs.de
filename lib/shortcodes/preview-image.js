...
import * as resizeImage from '../utils/resize-image.js';
import ConsoleLogger from '../utils/logger.js';

const log = new ConsoleLogger('preview');
...
export default async function (
  ...
  const inputDir = path.join(
    ...
    path.basename(page.inputPath, path.extname(page.inputPath))
  );

  const cacheKey = `${page.url}:${imageSrc}`;
  if (!resizeCache.has(cacheKey)) {
    log.info(`Resizing image ${page.outputPath} ${imageSrc}`);
    resizeCache.set(cacheKey, await resizeImage.resize(imageSrc, inputDir, path.dirname(page.outputPath), urlPath, theOptions, log));
  }
  const imageMetadata = resizeCache.get(cacheKey);
  if (imageMetadata == null) {
    log.warn(`Image not found, falling back to preview.svg: ${imageSrc}`);
    return getPreviewFigure(urlPath, alt ?? page.title);
  }
